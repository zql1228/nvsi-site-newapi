...
const getSystemPath = function() {
  ...
  if (_env === "development") {
    projectName = "/nvsidfapis/NVSIDF/";
    //path = `http://192.168.6.113:8086/NVSI/`,
    path = `https://chinavolunteer.mca.gov.cn/NVSIDF/`;
  }
...
const login = function(username, password) {
  ...
  //var purl = "/" + location.pathname.split("/")[1] + "/";
      ...
      Vue.utilscommit.requestOther(
        ...
        function(res) {
          ...
          for (var i = 0; i < temp.length; i++) {
            ...
            if (ts != "isallshow") window.Vue.common.deleteCookie(ts);
          }
          window.___sid = "";
          console.log(res, "已清缓存");
          Vues.toleaploginlogin();
        }
      ); //获取首页新闻
    }
    return null;
