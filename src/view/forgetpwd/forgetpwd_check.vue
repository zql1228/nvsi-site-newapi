...
export default {
  ...
  methods: {
    ...
    userinfoBack(result) {
      if (result) {
        if (result.data) {
          ...
          if (this.id == 1) this.forget_email = this.userinfo.albp0081 || result.data.albe0049; //邮箱
    ...
    // 取消
    nobtn() {
      this.$router.push({ name: "forgetpwd2", params: { tabtype: this.type } });
    },
    // 确定
    next_step(id) {
      // var params = null;
      if (id == 1 && this.getemail()) {
        ...
        if (this.email == this.forget_email) {
          ...
          this.$router.push({
            path: `/forgetpwd_checkout/${this.id}/${this.type}`,
          });
        } else {
