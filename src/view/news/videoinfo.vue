          ...
          <li class="col">
            <div class="router">
              ...
              <a
                ...
                v-show="firstmune"
                @click="getUrl(firstmune.id, 3)"
                v-text="firstmune.albx6002"
              ></a>
              <a
                href="javascript:void(0);"
                v-if="secondmune"
                @click="getUrl(secondmune.id, 4)"
                      ...
                      <div class="col v-t">
                        ...
                        <div class="message-child">
                          <div
                            ...
                            :key="key"
                          >
                            <!--   <h3 class="activity-tit" style="margin-top: 10px;font-size: 16px!important;font-weight: inherit!important;">
                                <a href="javascript:void(0);" @click="toDetailInfo(el2.volnum1,el2.area1,el2.orgid1,el2.area3)"><em v-text="el2.name1 || el2.name3"></em></a>
                                <span>回复</span>
                                <a href="javascript:void(0);" @click="toDetailInfo(el2.volnum2,el2.area2,el2.orgid2,el2.area4)"><em v-text="el2.name2 || el2.name4"></em></a>
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      this.utilscommit.request("nvsi_getUserInfo", null, this.userBack); //用户信息
    },
    videoinfoBack(data) {
      //获取新闻详情
      if (data && data.data) {
        this.videoinfo = data.data;
        this.firstmune = this.videoinfo.menulist[0];
        //二级栏目
        if (this.videoinfo.menulist[1])
          this.secondmune = this.videoinfo.menulist[1];
      }
    },
    userBack(result) {
      //获取用户信息
    ...
    toDetailInfo: function(volnum, albp0020, orgid, albe0012) {
      if (volnum) {
        window.open(
          this.$router.resolve({
            name: "volinfo",
            params: { data1: volnum, data2: albp0020 }
          }).href
        );
      } else if (orgid) {
        window.open(
          this.$router.resolve({
            name: "groupInfo",
            params: { data1: orgid, data2: albe0012 }
          }).href
        );
      }
    },
